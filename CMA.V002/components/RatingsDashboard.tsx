    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex justify-center mb-4">
        <div className="bg-gray-100 p-1.5 rounded-xl inline-flex items-center shadow-inner">
           ...
           <button 
             ...
             className={`flex items-center gap-2 px-6 py-2.5 rounded-lg text-sm font-bold transition-all
               ${dashboardMode === 'DEADLINES' ? 'bg-white text-brand-600 shadow-md ring-1 ring-gray-200' : 'text-gray-500 hover:text-gray-800'}`}
           >
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className={`p-6 rounded-xl border shadow-sm flex items-center justify-between
          ${dashboardMode === 'RATINGS' ? 'bg-white border-yellow-100' : 'bg-white border-brand-100'}`}>
           <div>
             <p className={`font-bold text-sm uppercase mb-1 ${dashboardMode === 'RATINGS' ? 'text-yellow-800' : 'text-brand-800'}`}>
             ...
             <h3 className="text-3xl font-extrabold text-gray-800 flex items-center gap-2">
               ...
               {dashboardMode === 'RATINGS' ? <Star className="text-yellow-400 fill-yellow-400" size={24}/> : <CheckCircle className="text-brand-500" size={24}/>}
             </h3>
           </div>
           <div className={`p-3 rounded-full shadow-sm bg-gray-50 ${dashboardMode === 'RATINGS' ? 'text-yellow-500' : 'text-brand-500'}`}>
             <TrendingUp size={24} />
           </div>
        </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
         <div className="flex items-center gap-2 mb-4">
            <BarChart2 className={dashboardMode === 'RATINGS' ? "text-yellow-600" : "text-brand-600"} size={20} />
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
          <div className="flex justify-between items-center mb-4">
             <h3 className="text-lg font-bold text-gray-800 flex items-center gap-2">
               <Medal className={dashboardMode === 'RATINGS' ? "text-yellow-500" : "text-brand-500"} size={20} /> 
               {dashboardMode === 'RATINGS' ? 'Ranking de Qualidade' : 'Ranking de Pontualidade'}
             </h3>
          </div>
          <div className="space-y-3">
            {currentData.map((tech, index) => (
               <div key={tech.id} onClick={() => setSelectedTechId(selectedTechId === tech.id ? null : tech.id)} className={`flex items-center justify-between p-3 rounded-lg border transition-all cursor-pointer ${selectedTechId === tech.id ? 'bg-brand-50 border-brand-500' : 'bg-white border-gray-100 hover:bg-gray-50'}`}>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden flex flex-col animate-fade-in">
            <div className="p-5 border-b border-gray-50 bg-gray-50/50 flex items-center gap-4 relative">
              ...
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                   <span className="font-bold text-brand-700">{dashboardMode === 'RATINGS' ? (techToDisplay as TechnicianStats).average.toFixed(1) : (techToDisplay as DeadlineStats).complianceRate.toFixed(0) + '%'}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2">
               {dashboardMode === 'RATINGS' ? (
                 ...
                 <>
                   <div className="p-6 border-r border-gray-100 text-center"><h4 className="text-3xl font-bold text-brand-600">{(techToDisplay as DeadlineStats).complianceRate.toFixed(1)}%</h4><p className="text-xs font-bold text-gray-500 uppercase">No Prazo</p></div>
